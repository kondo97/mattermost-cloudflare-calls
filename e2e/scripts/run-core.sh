...
docker exec \
	${CONTAINER_SERVER}1 \
	sh -c "/mattermost/bin/mmctl plugin disable com.mattermostl.cloudflare.calls && sleep 2 && /mattermost/bin/mmctl config patch /mattermost/config-patch-core.json && sleep 2 && /mattermost/bin/mmctl plugin enable com.mattermost.calls"
